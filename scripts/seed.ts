/**
 * Seed 73 competition teams for Judge Portal.
 * Run: npm run seed
...
async function run() {
  ...
  // Delete all existing data from relevant collections
  console.log('Clearing existing data...');
  const JudgeTeam = mongoose.connection.collection('judgeteams');
  const Score = mongoose.connection.collection('scores');
  const Team = mongoose.connection.collection('teams');
  
  await JudgeTeam.deleteMany({});
  await Score.deleteMany({});
  await Team.deleteMany({});
  
  console.log('Cleared all existing data from judgeteams, scores, and teams collections');
  ...
  await JudgeTeam.insertMany(teams);
  ...
  const byLab = teams.reduce<Record<string, number>>((acc, t) => {
    acc[t.labId] = (acc[t.labId] ?? 0) + 1;
    return acc;
  }, {});
